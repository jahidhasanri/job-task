      ...
      <div className="grid xl:grid-cols-4 md:grid-cols-2 gap-7.5">
        {(showAll ? products : products.slice(0, 4)).map((product) => (
          <div key={product.id} className="border rounded-lg  flex flex-col items-center xl:max-w-67.5 h-80.5">
             <div className="w-full h-52.5 bg-[#F5F5F5] rounded-lg flex flex-col justify-between ">
        ...
        <div className="w-full flex justify-center items-center h-full overflow-hidden">
          <Image
            ...
            width={190}
            height={150}
            className="scale-120 object-contain transition-transform duration-300  mx-10 mt-5.5 mb-1.75"
          />
        </div>
        <Link
          href={`/product/${product.id}`}
          className="mt-3 w-full h-10.25 flex items-center justify-center bg-[#0F59B9] text-white rounded-b-lg cursor-pointer"
        >
          Add to Cart
        </Link>
      </div>
