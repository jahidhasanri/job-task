    ...
    <div className="xl:max-w-292.5 mx-auto">
      ...
      <div className="grid xl:grid-cols-4 md:grid-cols-2 gap-7.5">
        {(showAll ? products : products.slice(0, 4)).map((product) => (
          <div key={product.id} className="border rounded-lg  flex flex-col items-center xl:max-w-67.5 h-80.5">
             <div className="w-full h-62.5 bg-[#F5F5F5] rounded-lg flex flex-col justify-between p-4">
        
        {/* Image Zoomed */}
        <div className="flex justify-center border items-center h-full overflow-hidden">
          <Image
            src={product.image}
            alt={product.name}
            width={190}
            height={180}
            className="scale-120 object-contain transition-transform duration-300"
          />
        </div>
        <Link
          href={`/product/${product.id}`}
          className="mt-3 w-full h-10.25 flex items-center justify-center bg-blue-600 text-white rounded cursor-pointer"
        >
          Add to Cart
        </Link>
      </div>
