...
export default function BestSellingProducts() {
  ...
  {(showAll ? products : products.slice(0, 4)).map((product) => (
    <div
      key={product.id}
      className="border rounded-lg flex flex-col items-center xl:max-w-67.5 h-80.5"
    >
      {/* Image Wrapper */}
      <div className="w-full h-62.5 bg-[#F5F5F5] rounded-lg flex flex-col justify-between p-4">
        ...
        <div className="flex justify-center items-center h-full overflow-hidden">
          <Image
            ...
            width={190}
            height={180}
            className="scale-110 object-contain transition-transform border duration-300"
          />
        </div>
        ...
        <Link
          ...
          className="mt-3 w-full h-14 flex items-center border justify-center bg-blue-600 text-white rounded cursor-pointer"
        >
          Add to Cart
        </Link>
      </div>
