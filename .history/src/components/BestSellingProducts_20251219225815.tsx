      ...
      <div className="grid xl:grid-cols-4 md:grid-cols-2 gap-7.5">
        {(showAll ? products : products.slice(0, 4)).map((product) => (
          <div key={product.id} className="border rounded-lg  flex flex-col items-center xl:max-w-67.5 h-80.5">
            <div className="w-full h-62.5 bg-[#F5F5F5] rounded-lg">
            <Image src={product.image} alt={product.name} width={190} height={180} className="border mx-10 mt-5.5 mb-1.75"/>
            <Link href={`/product/${product.id}`} className="mt-2 px-3 py-1 w-full h-10.25 bg-blue-600 text-white rounded">
              Add to Cart
            </Link>
