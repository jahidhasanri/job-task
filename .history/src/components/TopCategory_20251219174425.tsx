...
export function TopCategory() {
  const categories = [
  { title: "Phones", image: "/Category-CellPhone.png", bg: "bg-[#bdecf9] hover:bg-red-100", border:"border" },
  { title: "Computers", image: "/Category-Computer.png", bg: "bg-[#f5c90a] hover:bg-blue-100", border:"border" },
  { title: "SmartWatch", image: "/Category-SmartWatch.png", bg: "bg-[#cbfeb0] hover:bg-green-100", border:"border" },
  { title: "Camera", image: "/Category-Camera.png", bg: "bg-[#91bef8] text-white hover:bg-purple-100", border:"border" },
  { title: "Headphones", image: "/Category-Headphone.png", bg: "bg-[#ff974c] hover:bg-yellow-100", border:"border" },
  ...
  return (
    ...
    <div className="xl:w-113.5 h-2.75  bg-[#1163CF] mt-3.75 mb-21.5"></div>
      ...
      {/* Categories */}
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-7.5 xl:max-w-313 xl-max-h-36.25 border">
        {categories.map((cat) => (
          <Card
            key={cat.title}
            className={`${cat.bg} transition rounded-none cursor-pointer xl:w-42.5`}
          >
