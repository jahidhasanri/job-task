...
export function TopCategory() {
  ...
  return (
    ...
    <div className="xl:w-113.5 h-2.75  bg-[#1163CF] mt-3.75 mb-21.5"></div>
      ...
      {/* Categories */}
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-[-10px] xl:max-w-313 xl-max-h-36.25 border">
        {categories.map((cat) => (
          <Card
            key={cat.title}
            className={`${cat.bg} ${cat.border} transition rounded-[5px]  cursor-pointer xl:w-42.5 h-36.25`}
          >
            <CardContent className="flex flex-col items-center justify-center  ">
              <Image
                ...
                height={56}
              />
              <p className="text-sm font-medium mt-4">{cat.title}</p>
            </CardContent>
          </Card>
        ))}
      </div>
