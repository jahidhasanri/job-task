  ...
  return (
    <div className="xl:max-w-292.5 mx-auto">
      ...
      <div className="grid xl:grid-cols-4 md:grid-cols-2 gap-7.5">
        ...
        <div className="w-full h-25 border justify-center items-center h-full overflow-hidden">
          <Image
            src={product.image}
            alt={product.name}
            width={190}
            height={100}
            className="scale-100 object-contain transition-transform duration-300 border mx-10 mt-5.5 mb-1.75"
          />
        <Link
          href={`/product/${product.id}`}
          className="mt-3 w-full h-10 flex items-center justify-center bg-[#0F59B9] text-white rounded-b-lg cursor-pointer"
        >
          Add to Cart
        </Link>
        </div>
