...
export function TopCategory() {
  ...
  return (
    ...
    <div className="xl:w-113.5 h-2.75  bg-[#1163CF] mt-3.75 mb-21.5"></div>
      ...
      {/* Categories */}
<div className="flex flex-wrap xl:flex-nowrap gap-x-7.5 xl:max-w-313 border xl:mb-15.5">
  {categories.map((cat) => (
    <div
      key={cat.title}
      className={`${cat.bg} ${cat.border} rounded-[5px] cursor-pointer w-42.5 h-36.25 flex flex-col items-center justify-center shrink-0`}
    >
      <Image
        src={cat.image}
        alt={cat.title}
